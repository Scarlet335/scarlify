...
export default function StudentSidebar({ isMobile = false, onClose }: StudentSidebarProps) {
  ...
  return (
    <aside
      className={`
        ...
        ${isMobile ? 'w-64 shadow-2xl' : collapsed ? 'w-20' : 'w-64'}
    ...
    >
      ...
      {/* Logo */}
      <div className={`h-16 flex items-center border-b border-gray-200 dark:border-gray-800 px-4 gap-2 ${collapsed && !isMobile ? 'justify-center px-0' : ''}`}>
        ...
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              ...
              {(!collapsed || isMobile) && <span className="flex-1 truncate text-left">{item.label}</span>}
            </Link>
          );
        })}
      </nav>
      ...
      <div className="border-t border-gray-200 dark:border-gray-800 p-3 space-y-1">
        <div className={`flex items-center ${collapsed && !isMobile ? 'justify-center' : 'justify-between'} px-3 py-2`}>
          {(!collapsed || isMobile) && <span className="text-sm text-gray-500">Dark Mode</span>}
          <DarkModeToggle />
        </div>
        ...
        <Link
          ...
          onClick={isMobile && onClose ? onClose : undefined}
          className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors ${collapsed && !isMobile ? 'justify-center' : ''}`}
        >
        ...
        <button
          ...
          className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium text-red-600 hover:bg-red-50 dark:hover:bg-red-950/30 transition-colors ${collapsed && !isMobile ? 'justify-center' : ''}`}
        >
