...
import { 
  ...
  Bot, User, Copy, Volume2, VolumeX, Mic, Plus, Camera, Sparkles,
  ChevronLeft
} from 'lucide-react';
import { useVoiceAssistant } from '@/hooks/useVoiceAssistant';

// Types only - no browser libraries imported at top level
interface Message {
    id: string;
    role: 'user' | 'assistant';
    content: string;
...
export default function ScarlifyAssistPage() {
    ...
    // Client-side only text extraction functions with dynamic imports
        ...
        return (
            <div className="min-h-screen flex items-center justify-center bg-gray-50 dark:bg-gray-900">
                <div className="text-center p-8 max-w-md">
                    ...
                    <h1 className="text-2xl font-bold text-gray-900 dark:text-white mb-2">Daily Limit Reached</h1>
                    <p className="text-gray-600 dark:text-gray-400 mb-6">You've used your 6 free coaching questions today.</p>
    ...
    return (
        <div className="min-h-screen bg-gray-50 dark:bg-gray-900 flex">
            {/* Sidebar */}
            <div className={`${sidebarOpen ? 'w-72' : 'w-0'} bg-white dark:bg-gray-800 border-r border-gray-200 dark:border-gray-700 transition-all duration-300 overflow-hidden flex flex-col`}>
                ...
                <div className="flex-1 overflow-y-auto p-2">
                    <p className="text-xs text-gray-400 dark:text-gray-500 px-2 mb-2">Recent Chats</p>
                    {conversations.map(conv => (
                        <div key={conv.id} className="p-2 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-lg cursor-pointer text-sm truncate text-gray-700 dark:text-gray-300">
                            {conv.title}
                        </div>
                    ))}
                </div>
            ...
            <div className="flex-1 flex flex-col">
                ...
                <header className="bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 px-4 py-3 flex items-center justify-between sticky top-0 z-10">
                    <div className="flex items-center gap-3">
                        ...
                        >
                            <ChevronLeft className={`w-5 h-5 text-gray-600 dark:text-gray-400 transition-transform ${sidebarOpen ? 'rotate-0' : 'rotate-180'}`} />
                        </button>
                        <div className="flex items-center gap-2">
                            <Sparkles className="w-6 h-6 text-primary" />
                            <h1 className="font-bold text-lg text-gray-900 dark:text-white">Scarlify Assist</h1>
                        </div>
                    </div>
                    <div className="flex items-center gap-2">
                        <span className={`text-sm px-3 py-1 rounded-full ${isPremium ? 'bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400' : 'bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-400'}`}>
                ...
                <div className="border-t border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 p-4">
                        ...
                        <div className="flex flex-wrap gap-2 mb-3">
                            {selectedImages.map((img, i) => (
                                <div key={`img-${i}`} className="relative">
                                    <img src={URL.createObjectURL(img)} alt="preview" className="w-16 h-16 object-cover rounded border border-gray-200 dark:border-gray-600" />
                            ...
                            {selectedFiles.map((file, i) => (
                                <div key={`file-${i}`} className="bg-gray-100 dark:bg-gray-700 rounded-lg px-3 py-2 text-sm flex items-center gap-2 text-gray-700 dark:text-gray-300">
                    ...
                    <div className="flex gap-2 items-end">
                        <div className="flex gap-1">
                            ...
                            <button onClick={() => imageInputRef.current?.click()} className="p-2 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700" title="Upload image">
                            ...
                            <button onClick={() => fileInputRef.current?.click()} className="p-2 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700" title="Upload PDF">
                        ...
                        <textarea
                            ...
                            rows={2}
                            className="flex-1 border border-gray-300 dark:border-gray-600 rounded-lg p-2 resize-none focus:outline-none focus:ring-2 focus:ring-primary bg-white dark:bg-gray-700 text-gray-900 dark:text-white"
                            disabled={loading}
                        />
